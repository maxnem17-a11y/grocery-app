...
**RLS hardening for the remaining anon-writable tables.** `recipes` + `tesco_skus` were locked down 2026-06-09 (read + scoped `source_page` update only). The other client-written tables — `pantry_items`, `receipts`, `receipt_items`, `cooked_log` (and read-only `household_allergens`) — have RLS *enabled* but with permissive `anon` policies, so anyone with the public anon key can still write/delete them. To close this without adding login: route their writes through trusted Edge Functions (service-role, like `ingest-receipt` already does) and tighten each table's anon policy to read-only — OR accept the keyless-personal-app trade-off. `pantry_items` is the most-written (toggles, qty, replenishment, expiry) so it's the biggest refactor; `cooked_log` is small. Bigger change than the recipes/tesco_skus lockdown. Open when the app stops being a single-trusted-user tool or before any wider sharing.
